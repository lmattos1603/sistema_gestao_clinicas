...
var data = [],
  ...
  @foreach($agenda as $a)
  @php
    $y = date('Y', strtotime($a->data));  //pega ano
    $m = date('m', strtotime($a->data)) -1; //pega mes
    $d = date('d', strtotime($a->hora));
    $h = date('H', strtotime($a->hora));
    $i = date('i', strtotime($a->data));

    $hf = date('H', strtotime($a->hora) + 60*60);
  @endphp
  data.push({ title: '{{ $a->cliente }}', start: new Date({{ $y }}, {{ $m }}, {{ $d }}, {{ $h }}, {{ $i }}), end: new Date({{ $y }}, {{ $m }}, {{ $d }}, {{ $hf }}, {{ $i }}), allDay: false, text: '{{ $a->titulo }}'  });
  @endforeach
